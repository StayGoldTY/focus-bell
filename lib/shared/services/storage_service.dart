...
class StorageService {
  ...
  int get focusDuration =>
      _prefs.getInt('focusDuration') ?? AppConstants.defaultFocusDurationMinutes;
  Future<void> setFocusDuration(int v) => _prefs.setInt('focusDuration', v);

  // 休息时长（分钟）
  int get breakDuration =>
      _prefs.getInt('breakDuration') ?? AppConstants.defaultBreakDurationMinutes;
  ...
  Future<void> setMicroRestSeconds(int v) => _prefs.setInt('microRestSeconds', v);
  ...
  // 选中的提示音 ID
  String get selectedSoundId => _prefs.getString('selectedSoundId') ?? 'singing_bowl';
  Future<void> setSelectedSoundId(String v) => _prefs.setString('selectedSoundId', v);

  // 随机提示音模式
  bool get randomSoundMode => _prefs.getBool('randomSoundMode') ?? false;
  Future<void> setRandomSoundMode(bool v) => _prefs.setBool('randomSoundMode', v);

  // 震动开关
  bool get vibrationEnabled => _prefs.getBool('vibrationEnabled') ?? true;
  Future<void> setVibrationEnabled(bool v) => _prefs.setBool('vibrationEnabled', v);

  // 科学贴士显示
  bool get showScienceTips => _prefs.getBool('showScienceTips') ?? true;
  Future<void> setShowScienceTips(bool v) => _prefs.setBool('showScienceTips', v);
  ...
  Future<void> setColorSchemeId(String v) => _prefs.setString('colorSchemeId', v);
  ...
  Future<void> setAmbientVolume(double v) => _prefs.setDouble('ambientVolume', v);

  // 统计数据：总专注秒数
  int get totalFocusSeconds => _prefs.getInt('totalFocusSeconds') ?? 0;
  Future<void> setTotalFocusSeconds(int v) => _prefs.setInt('totalFocusSeconds', v);

  // 统计数据：完成轮次
  int get completedSessions => _prefs.getInt('completedSessions') ?? 0;
  Future<void> setCompletedSessions(int v) => _prefs.setInt('completedSessions', v);

  // 今日专注秒数
  int get todayFocusSeconds => _prefs.getInt('todayFocusSeconds') ?? 0;
  Future<void> setTodayFocusSeconds(int v) => _prefs.setInt('todayFocusSeconds', v);
